pub use crate::term::{TermReadDestination, TermWriteDestination, TryCloneAsFile};
...
pub struct Console {
    ...
    pub stderr_use_color: bool,
}

impl Console {
    pub fn new(stdin_fd: RawFd, stdout_fd: RawFd, stderr_fd: RawFd) -> Console {
    ...
    pub fn write_stdout(&mut self, content: &[u8]) -> Result<(), std::io::Error> {
        let mut stdout = self.stdout_handle.as_ref().unwrap();
        stdout.write_all(content)?;
        stdout.flush()
    }

    pub fn write_stderr(&mut self, content: &[u8]) -> Result<(), std::io::Error> {
        let mut stderr = self.stderr_handle.as_ref().unwrap();
        stderr.write_all(content)?;
        stderr.flush()
    }

    pub fn stdin_as_raw_fd(&self) -> RawFd {
        self.stdin_handle.as_ref().unwrap().as_raw_fd()
    }

    pub fn stderr_set_use_color(&mut self, use_color: bool) {
        self.stderr_use_color = use_color;
    }

    pub fn stdout_as_raw_fd(&self) -> RawFd {
        self.stdout_handle.as_ref().unwrap().as_raw_fd()
    }

    pub fn stderr_as_raw_fd(&self) -> RawFd {
...
impl Destination {
    ...
    > {
        ...
        match console {
            InnerDestination::Console(console) => {
                Ok(crate::term::TermDestination::new(console, self.clone()))
            }
            _ => unreachable!(),
        }
    }
    ...
    ///
    pub fn exclusive_clear(&self, console: Console) {
        let mut destination = self.0.lock();
